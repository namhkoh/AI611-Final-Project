...
from llava.conversation import Conversation#simple_conv_multimodal
...
conv = Conversation(
    system="",                  # 시스템 메시지(초기 프롬프트)
    roles=["User", "Assistant"],# 역할(role) 리스트
    messages=[],                # 아직 대화 내역이 없으므로 빈 리스트
    offset=0                    # offset은 적절한 기본값으로
)

PROMPT = conv.get_prompt() + "Human: "


def load_llava(params_path):
    # load model : params_path = "llava-hf/llava-1.5-7b-hf"
    disable_torch_init()
    # tokenizer = AutoTokenizer.from_pretrained(params_path)
    # model = AutoModelForCausalLM.from_pretrained(params_path, torch_dtype=torch.float16).cuda()
    tokenizer = AutoProcessor.from_pretrained(params_path)
    model = LlavaForConditionalGeneration.from_pretrained(params_path, torch_dtype=torch.float16).cuda()
